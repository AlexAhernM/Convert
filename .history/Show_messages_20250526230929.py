...
def show_messages(self, messages):
    ...
    x= 20, y =20
    if msg_dxf:
        self.save_dxf = customtkinter.CTkButton(self.show_files_frame, text=msg_dxf, text_color='firebrick1', fg_color="transparent", hover_color=self.checkbox_type.get("fg_color"), border_width=0)
    ...
    if msg_shp:
        self.save_shp = customtkinter.CTkButton(self.checkbox_type, text=msg_shp, text_color='blue', fg_color="transparent", hover_color=self.checkbox_type.get("fg_color"), border_width=0)
        self.label_shp = customtkinter.CTkLabel(self.show_files_frame, text= 'Shapefile has been generated in the following path : ')
        
        if ruta_shp:
            self.label_shp.place(x=x, y = y+20)
    ...
    if msg_xlx:
        self.save_xlx = customtkinter.CTkButton(self.checkbox_type, text=msg_xlx, text_color='PaleGreen4', fg_color="transparent", hover_color=self.checkbox_type.get("fg_color"), border_width=0)
        self.label_xlx = customtkinter.CTkLabel(self.show_files_frame, text= 'Excel file has been generated in the following path : ')
        
        if ruta_xlx:
            
            self.label_xlx.place(x=x, y = y+20)
