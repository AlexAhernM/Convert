...
def crear_files(self, doc, ruta_archivo_kml, coords, utm_points_list, layer_names):
    ...
    if dxf_selected or shp_selected or pdf_selected:
        ...
        if  pdf_selected:
            ...
            for entity in msp:
                if entity.dxftype() == 'LINE':
                    ...
                    if entity.vertices:
                        ...
                        if len(x) > 2 and x[0] == x[-1] and y[0] == y[-1]:  # Si es un círculo
                            ax.fill(x[:-1], y[:-1], color='black')  # Dibujar el círculo relleno
                            ax.annotate(entity.dxf.layer, (sum(x[:-1])/len(x[:-1]), sum(y[:-1])/len(y[:-1])), 
                                        ha='center', fontsize=6)  # Colocar el nombre de la capa con tamaño de fuente reducido
                        else:
                            ax.plot(x, y, 'k-', lw=0.5)  # Dibujar la polilínea
                            ax.plot(x[0], y[0], 'ko', markersize=2)  # Dibujar un punto en el inicio de la polilínea con tamaño reducido
                            ax.annotate(entity.dxf.layer, (x[0], y[0]), textcoords="offset points", xytext=(0, 5), ha='center', fontsize=6)  # Colocar el nombre de la capa con tamaño de fuente reducido
                    ...
                    ax.annotate(entity.dxf.layer, (entity.dxf.location[0], entity.dxf.location[1]), textcoords="offset points", xytext=(0, 15), ha='center')
            ...
            ax_table = fig.add_axes([0.1, 0.05, 0.8, 0.1])
            ax_table.axis('off')
