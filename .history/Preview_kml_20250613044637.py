...
from Transforma import parseo,  obtener_coordenadas_capas, encontrar_placemark, procesar_placemark, get_zoom_level
...
def procesar_archivo_kml(self):
    ...
    if ruta_archivo_kml: 
        ...
        doc, utm_coordscoords_dec, layers, lat_centro, lon_centro, radio = obtener_coordenadas_capas(root_element_xml, altitud_value)
        ...
        confirmar_localizacion (self, doc, ruta_archivo_kml, utm_coordslayers, coords_dec)
        
    else:
        messagebox.showerror("Error", "Por favor, seleccione un archivo KML")
            
    return doc, ruta_archivo_kml, utm_coordslayers, coords_dec, altitud_value
        
def confirmar_localizacion(self, doc, ruta_archivo_kml, utm_coordslayers, coords_dec):
    ...
    def respuesta_confirmacion(respuesta):
        if respuesta == "incorrecta":
            ...
            generate_files(self, doc, ruta_archivo_kml, utm_coordslayers, coords_dec) or ("", "", "")
...
def generate_files(self, doc, ruta_archivo_kml, utm_coordslayers, coords_dec):
    def on_click():
        messages = generate_files_intermediate(self, doc, ruta_archivo_kml, utm_coordslayers, coords_dec)
