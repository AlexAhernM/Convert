...
def procesar_placemark(placemark, obtener_elevacion_valor, coords, layers, coords_dec):
    ...
    if mg is not None:
        coords,  coords_dec, layers = procesar_multigeometrias(mg, layer_name, obtener_elevacion_valor, coords, layers, coords_dec)
    elif coord is not None:
        coords,  coords_dec, layers = obtener_coordenadas_utm(coord, layer_name, obtener_elevacion_valor, coords, layers, coords_dec)

    return coords, coords_dec, layers, layer_name
...
def obtener_coordenadas_capas(root_element_xml, altitude_value):
    ...
    for placemark in encontrar_placemark(root_element_xml):
        coords, coords_dec, layers, layer_name = procesar_placemark(placemark, altitude_value, coords, layers, coords_dec)
    ...
    for coords, layer_name in zip(coords_list, layer_names):
...
def procesar_multigeometrias(geoms, layer_name, obtener_elevacion_valor, coords, layers, coords_dec):
    ...
    return coords_total, coords_total, coords_dec_total, layers_total
...
def agregar_polilinea(coords, layer_name, doc):
    ...
    Parámetros:
    coords (list): Lista de puntos UTM.
