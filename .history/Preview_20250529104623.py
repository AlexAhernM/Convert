...
from Convert_file import convert_files
...
def confirmar_localizacion(self, doc, ruta_archivo_kml, coords, layers, coords_dec):
    ...
    self.label_mappreview = ctk.CTkLabel(self.ventana_segunda, text='Vision Preliminar Area Geografica a Procesar',
...
def update_preview(self, lat_centro, lon_centro, zoom_start, root, altitud_value):
    ...
    self.label3 = ctk.CTkLabel(self.ventana_segunda, text='', width=700, height=500)
...
def generate_files(self, doc, ruta_archivo_kml, coords, layers, coords_dec):
    ...
    self.boton_limpiar = ctk.CTkButton(self.ventana_segunda, text="CLEAN", command=lambda: widgets_destroy(self),
                              fg_color='pink', text_color='Blue' , width = 200, height=25 ,corner_radius=6)
    self.boton_limpiar.place(x=80, y = 460)            