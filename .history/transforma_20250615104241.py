...
def obtener_coordenadas_capas(root_element_xml, altitude_value):
    ...
    resultados = obtener_maximos_minimos(utm_points, coords_dec)
...
def obtener_maximos_minimos(utm_points, coords_dec):
    ...
    lat_min = min(utm_points, key=lambda x: x[0])[0]
    lat_max = max(utm_points, key=lambda x: x[0])[0]
    lon_min = min(utm_points, key=lambda x: x[1])[1]
    lon_max = max(utm_points, key=lambda x: x[1])[1]
    
    print ('lat mim =', lat_min)
    print ('lat max =', lat_max)
    print ('lon mim =', lon_min)
