            ...
            for entity in msp:
                if entity.dxftype() == 'LINE':
                    ...
                    if entity.vertices:
                        ...
                        if xdata and xdata[0][1] == 'CIRCULO':# Es un círculo creado en el script TRANFORMA
                            ax.fill(x[:-1], y[:-1], color='black')  # Dibujar el círculo relleno
                            ax.annotate(entity.dxf.layer, (sum(x[:-1])/len(x[:-1]), sum(y[:-1])/len(y[:-1])), 
                                        ha='center', fontsize=4)  # Colocar el nombre de la capa
                        else:
                            ax.plot(x, y, 'k-', lw=0.5)  # Dibujar la polilínea
                            #ax.annotate(entity.dxf.layer, (x[0], y[0]), textcoords="offset points", xytext=(0, 5), ha='center', fontsize=6)  # Colocar el nombre de la capa
