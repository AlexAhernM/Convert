from rbutton_cor import convert_cor

import select_kml as ctk
from select_kml import filedialog
...
def convert_cor(self):
    class Rbutton:
        def __init__(self, master, values):
            self.rbuttons = []
            self.labels = []
            self.var = select_kml.IntVar()
            

            for i, value in enumerate(values):
                label = select_kml.CTkLabel(master, text=value, fg_color='red')
                Rbutton = select_kml.CTkRadioButton(master, variable=self.var, value=i, text='')
                y=80
                x=30
                if i==0 :
    ...
    self.zona_var = select_kml.IntVar(value=1)
    
    self.slider = select_kml.CTkSlider(self, from_=1, to=60, variable=self.zona_var, number_of_steps=59)
    self.slider.place(x=150, y=165)
    
    self.label_valor = select_kml.CTkLabel(self, text = 'Zona Geografica')
    self.label_valor.place(x=190, y=140)
    
    self.label_valor = select_kml.CTkLabel(self, textvariable = self.zona_var)
    self.label_valor.place(x=243, y=178)
    

    self.hemi_var = select_kml.StringVar(value="Norte")
    
    self.label_hemi = select_kml.CTkLabel(self, text='Hemisferio')
    self.label_hemi.place(x=430, y=140)   
                                        
    self.checkbox_hemi = select_kml.CTkComboBox(self, values=["Norte", "Sur"], variable=self.hemi_var)
