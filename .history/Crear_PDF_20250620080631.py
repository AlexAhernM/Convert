...
def crear_pdf(ruta_archivo_salida_dxf):
    ...
    distancia_minima = next((distancia for rango, distancia in distancias.items() if rango[0] <= escala_num <= rango[1]), None)
    ...
    # Colocar los textos con la distancia ajustada
    if entity.has_xdata('ACAD'):
        textos_existentes = []
        for entity in msp:
            if entity.dxftype() == 'POLYLINE':
                ...
                for texto in textos_existentes:
                    distancia_real = ((x_texto - texto[0]) ** 2 + (y_texto - texto[1]) ** 2) ** 0.5
