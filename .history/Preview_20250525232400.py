...
def confirmar_localizacion(self, doc, ruta_archivo_kml, coords, layers, coords_dec):
    ...
    label_mappreview.place(x=580, y=100) 
    
    
    
    self.boton_looks_good = ctk.CTkButton(self.label3, text="Looks good", command=lambda: respuesta_confirmacion("correcta"),
                                                  fg_color='gray69', text_color='Blue' , width = 100, height=25,  corner_radius=6)
                                 
    self.boton_looks_good.pack(side='left') 
    
    
    self.boton_no_good = ctk.CTkButton(self.label3, text="select another file", command=lambda: respuesta_confirmacion("incorrecta"),
                              fg_color='gray69', text_color='Blue' , width = 100, height=25 ,corner_radius=6)
    self.boton_no_good.pack(side='right') 
...
def update_preview(self, lat_centro, lon_centro, zoom_start, root, altitud_value):
    ...
    self.mapa_label = ctk.CTkLabel(self.ventana_segunda, text='', width=600, height=400)
    self.mapa_label.place(x=580, y=140)
    
    self.mapa_tkinter = TkinterMapView(self.mapa_label)
