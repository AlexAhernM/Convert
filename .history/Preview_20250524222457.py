...
def confirmar_localizacion(self, doc, ruta_archivo_kml, coords, layers, coords_dec):
    ...
    label_mappreview = ctk.CTkLabel(self, text='Vision Preliminar Area Geografica a Procesar', 
                                                       fg_color='gray69', text_color='Blue' , width = 290, height=25, corner_radius=6)
    label_mappreview.place(row=0,column=0,padx=10, pady =(5,500)) 
    
    
    
    self.boton_looks_good = ctk.CTkButton(self.show_files_frame , text="Looks good", command=lambda: respuesta_confirmacion("correcta"),
                                                  fg_color='gray69', text_color='Blue' , width = 100, height=25,  corner_radius=6)
                                 
    self.boton_looks_good.pack(side='left') 
    
    
    self.boton_no_good = ctk.CTkButton(self.show_files_frame, text="select another file", command=lambda: respuesta_confirmacion("incorrecta"),
                              fg_color='gray69', text_color='Blue' , width = 100, height=25 ,corner_radius=6)
