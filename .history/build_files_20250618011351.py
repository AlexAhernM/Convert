...
import ezdxf
from Transforma import num_points
            ...
            for entity in msp:
                if entity.dxftype() == 'LINE':
                    ...
                    if entity.vertices:
                        ...
                        if len(x) == num_points:  # si es un circulo
                            ax.fill(x[:-1], y[:-1], color='snow')  # Dibujar el círculo relleno
                            ax.annotate(entity.dxf.layer, (sum(x[:-1])/len(x[:-1]), sum(y[:-1])/len(y[:-1])), 
                                        ha='center', fontsize=6)  # Colocar el nombre de la capa
                        else:
