...
def crear_dxf(self, doc, ruta_archivo_kml, coords, layers, coords_dec):
    ...
    if xlsx_selected:
        ...
        print ('coordenadas decimales =', coords_dec, 'capas =', layers)
        
        print ('coordenadas utm =', coords, 'capas =', layers)
        ...
        data = []
        for coord, layer in zip(coords, layers):
            for point, coord_dec in zip(coords, coords_dec[:len(coords)]):
                data.append([coord_dec[0], coord_dec[1], point[0], point[1], point[2], layers])
            coords_dec = coords_dec[len(coords):]
