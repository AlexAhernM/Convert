...
def procesar_archivo_kml(self):
    ...
    if ruta_archivo_kml: 
        ...
        doc, coords, coords_dec, layers, lat_centro, lon_centro, radio, utm_point_list, layer_names = obtener_coordenadas_capas(root_element_xml, altitud_value)
        ...
        confirmar_localizacion (self, doc, ruta_archivo_kml, coords, utm_point_list, layer_names)
...
def confirmar_localizacion(self, doc, ruta_archivo_kml, coords, utm_point_list, layer_names):
    ...
    def respuesta_confirmacion(respuesta):
        if respuesta == "incorrecta":
            ...
            generate_files(self, doc, ruta_archivo_kml, coords, utm_point_list, layer_names) or ("", "", "")
...
def update_preview(self, lat_centro, lon_centro, zoom_start, root_element_xml, altitud_value):
     # Destruye todos los widgets hijos de self.HomeImages (la imagen)
    for widget in self.HomeImages.winfo_children():
        widget.destroy()
    self.HomeImages.destroy()

    self.HomeImages = ctk.CTkLabel(self, text='', width=700, height=500)
    self.HomeImages.place(x=580, y=80)
    
    self.mapa_tkinter = TkinterMapView(self.HomeImages)
