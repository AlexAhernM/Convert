...
def preview_kml(self):
    ...
    if ruta_archivo_kml: 
        ...
        pichuka, altitud_value = parseo(ruta_archivo_kml, self.checkbox_altitude.get())
        doc, coords, coords_dec, layers, lat_centro, lon_centro, radio = convierte(pichuka, altitud_value)
        zoom_start = get_zoom_level(radio)
        print ('zoom=',  zoom_start, ', radio =', radio)
        update_preview(self, lat_centro, lon_centro, zoom_start, pichuka, altitud_value)
        self.buttton_preview_kml.configure(state= 'disabled')
