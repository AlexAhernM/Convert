...
def convert_cor(self):
    ...
    self.zona_var = customtkinter.IntVar(value=1)
    
    self.slider = customtkinter.CTkSlider(self, from_=1, to=60, variable=self.zona_var, number_of_steps=59)
    self.slider.place(x=150, y=165)
    
    self.label_valor = customtkinter.CTkLabel(self, text = 'Zona Geografica')
    self.label_valor.place(x=190, y=140)
    
    self.label_valor = customtkinter.CTkLabel(self, textvariable = self.zona_var)
    self.label_valor.place(x=243, y=178)
    

    self.hemi_var = customtkinter.StringVar(value="Norte")
    
    self.label_hemi = customtkinter.CTkLabel(self, text='Hemisferio')
    self.label_hemi.place(x=430, y=140)   
                                        
    self.checkbox_hemi = customtkinter.CTkComboBox(self, values=["Norte", "Sur"], variable=self.hemi_var)
