...
def procesar_placemark(placemark, obtener_elevacion_valor, coords, layers, coords_dec):
    ...
    if mg is not None:
        ...
        utm_points, coords, coords_dec, layers = obtener_coordenadas_utm(coord, layer_name, obtener_elevacion_valor, coords, layers, coords_dec)

    return utm_points, coords, coords_dec, layers, layer_name
    
def obtener_coordenadas_capas(root_element_xml, altitude_value):
    doc = ezdxf.new('R2013')
    coords = []  #corresponden a coordenas UTM
    layers = []
    coords_dec = []
    utm_points_list = []
    layer_names = []
